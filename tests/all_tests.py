...
import tempfile
import unittest
from bifrost import h5dm, axes_utils, datasets_utils
...
class TestHDF5DataManager(unittest.TestCase):
    ...
    def test_invalid_axis_creation(self):
        with self.assertRaises(ValueError):
            axes_utils.AxisObject.create_dict_inputs("", "valid_name", [1, 2, 3]) # empty group
        with self.assertRaises(ValueError):
            axes_utils.AxisObject.create_dict_inputs("valid_group", "", [1, 2, 3]) # empty name
        with self.assertRaises(TypeError):
            axes_utils.AxisObject.create_dict_inputs("valid_group", "valid_name", "invalid_values") # no axis values
        with self.assertRaises(ValueError):
            axes_utils.AxisObject.create_dict_inputs("valid_group", "valid_name", [[1, 2], [3, 4]]) # axis values are not a flat set of values
        with self.assertRaises(ValueError):
            axes_utils.AxisObject.create_dict_inputs("valid_group", "valid_name", [2, 1, 4, 3]) # axis values are not monotonically increasing
        with self.assertRaises(ValueError):
            axes_utils.AxisObject.create_dict_inputs("valid_group", "valid_name", [1, 1, 2, 2]) # axis values are not unique
        with self.assertRaises(TypeError):
            axes_utils.AxisObject.create_dict_inputs("valid_group", "valid_name", [1, 2, 3], "invalid_units") # invalid units

    def test_invalid_dataset_creation(self):
        with self.assertRaises(ValueError):
            datasets_utils.DatasetObject.create_dict_inputs("", "valid_name", [1, 2, 3]) # empty group
        with self.assertRaises(ValueError):
            datasets_utils.DatasetObject.create_dict_inputs("valid_group", "", [1, 2, 3]) # empty name
        with self.assertRaises(TypeError):
            datasets_utils.DatasetObject.create_dict_inputs("valid_group", "valid_name", "invalid_values") # no dataset values
        with self.assertRaises(TypeError):
            datasets_utils.DatasetObject.create_dict_inputs("valid_group", "valid_name", [1, 2, 3], "invalid_units") # invalid units

    def test_axis_creation(self):
        axis_values = np.arange(1000)
        dict_axis = axes_utils.AxisObject.create_dict_inputs("axis_group", "axis_name", axis_values, axes_utils.AxisUnits.NOT_SPECIFIED, notes="Test axis")
        ...
        self.assertEqual(dict_axis["units"], axes_utils.AxisUnits.NOT_SPECIFIED.value)
        self.assertEqual(dict_axis["notes"], "Test axis")

    def test_dataset_creation(self):
        dataset_values = np.random.rand(1000) * 100
        dict_dataset = datasets_utils.DatasetObject.create_dict_inputs("dataset_group", "dataset_name", dataset_values, datasets_utils.DatasetUnits.DIMENSIONLESS, notes="Test dataset")
        ...
        self.assertEqual(dict_dataset["units"], datasets_utils.DatasetUnits.DIMENSIONLESS.value)
    ...
    def test_add_data(self):
        ...
        dict_axis = axes_utils.AxisObject.create_dict_inputs("axis_group", "axis_name", axis_values, axes_utils.AxisUnits.NOT_SPECIFIED)
        dict_dataset = datasets_utils.DatasetObject.create_dict_inputs("dataset_group", "dataset_name", dataset_values, datasets_utils.DatasetUnits.NOT_SPECIFIED)
    ...
    def test_adding_multiple_datasets_with_a_shared_axis(self):
        ...
        axis_dict_1 = axes_utils.AxisObject.create_dict_inputs("axis_group_1", "axis_name_1", axis_values_1, axes_utils.AxisUnits.NOT_SPECIFIED)
        axis_dict_2 = axes_utils.AxisObject.create_dict_inputs("axis_group_2", "axis_name_2", axis_values_2, axes_utils.AxisUnits.NOT_SPECIFIED)
        dataset_dict_1 = datasets_utils.DatasetObject.create_dict_inputs("dataset_group", "dataset_1", dataset_values_1, datasets_utils.DatasetUnits.NOT_SPECIFIED)
        dataset_dict_2 = datasets_utils.DatasetObject.create_dict_inputs("dataset_group", "dataset_2", dataset_values_2, datasets_utils.DatasetUnits.NOT_SPECIFIED)
    ...
    def test_extending_dataset(self):
        ...
        new_axis_dict = axes_utils.AxisObject.create_dict_inputs("axis_group", "axis_name", new_axis_values, axes_utils.AxisUnits.NOT_SPECIFIED)
        new_dataset_dict = datasets_utils.DatasetObject.create_dict_inputs("dataset_group", "dataset_name", new_dataset_values, datasets_utils.DatasetUnits.NOT_SPECIFIED)
    ...
    def test_reindexing_dataset_to_get_values_with_global_axis_values(self):
        ...
        dataset_values = np.array([10, 20, 30])
        dict_axis = axes_utils.AxisObject.create_dict_inputs("axis_group", "axis_name", axis_values, axes_utils.AxisUnits.NOT_SPECIFIED)
        dict_dataset = datasets_utils.DatasetObject.create_dict_inputs("dataset_group_1", "dataset_name_1", dataset_values, datasets_utils.DatasetUnits.NOT_SPECIFIED)
        self.obj_h5dm.add(dict_dataset, [dict_axis])
        new_dataset_values = np.array([40, 50])
        new_axis_values = np.array([3, 4])
        new_axis_dict = axes_utils.AxisObject.create_dict_inputs("axis_group", "axis_name", new_axis_values, axes_utils.AxisUnits.NOT_SPECIFIED)
        new_dataset_dict = datasets_utils.DatasetObject.create_dict_inputs("dataset_group_2", "dataset_name_2", new_dataset_values, datasets_utils.DatasetUnits.NOT_SPECIFIED)
        self.obj_h5dm.add(new_dataset_dict, [new_axis_dict])
    ...
    def test_dataset_reindexing_expands_correctly(self):
        ...
        axis = axes_utils.AxisObject("group1", "axis1", axis_values)
        dataset = datasets_utils.DatasetObject("group1", "dataset1", dataset_values, [axis])
    ...
    def test_add_and_update_metadata(self):
        dict_axis = axes_utils.AxisObject.create_dict_inputs("axis_group", "axis_name", [0, 1, 2])
        dict_dataset = datasets_utils.DatasetObject.create_dict_inputs("dataset_group", "dataset_name", [10, 20, 30])
        self.obj_h5dm.add(dict_dataset, [dict_axis])
        self.obj_h5dm.update_global_axis_metadata("axis_group", "axis_name", units=axes_utils.AxisUnits.T_TURB, notes="Updated notes")
        updated_dataset = self.obj_h5dm.get_local_dataset("dataset_group", "dataset_name")
        self.assertEqual(updated_dataset["list_axis_dicts"][0]["units"], axes_utils.AxisUnits.T_TURB.value)
    ...
    def test_extend_existing_dataset(self):
        ...
        dict_dataset = datasets_utils.DatasetObject.create_dict_inputs("dataset_group", "dataset_name", [10, 20, 30])
        self.obj_h5dm.add(dict_dataset, [dict_axis])
        new_axis = axes_utils.AxisObject.create_dict_inputs("axis_group", "axis_name", [3, 4])
        new_data = datasets_utils.DatasetObject.create_dict_inputs("dataset_group", "dataset_name", [40, 50])
    ...
    def test_empty_axis_values(self):
        empty_axis_values = []
        with self.assertRaises(ValueError):
            axes_utils.AxisObject.create_dict_inputs("axis_group", "axis_name", empty_axis_values, axes_utils.AxisUnits.NOT_SPECIFIED)
    ...
    def test_axis_values_with_nan(self):
        ...
        with self.assertRaises(ValueError):
            axes_utils.AxisObject.create_dict_inputs("axis_group", "axis_name", axis_values, axes_utils.AxisUnits.NOT_SPECIFIED)

    def test_invalid_axis_unit_type(self):
        axis_values = np.arange(3)
        with self.assertRaises(TypeError):
            axes_utils.AxisObject.create_dict_inputs("axis_group", "axis_name", axis_values, "invalid_unit")

    def test_invalid_dataset_unit_type(self):
        dataset_values = np.random.rand(3)
        with self.assertRaises(TypeError):
            datasets_utils.DatasetObject.create_dict_inputs("dataset_group", "dataset_name", dataset_values, "invalid_unit")

    def test_adding_overlap_axis_values(self):
        axis_values_1 = np.array([1, 2, 3])
        axis_values_2 = np.array([2, 3, 4])
        dataset_values = np.random.rand(3)
        axis_dict_1 = axes_utils.AxisObject.create_dict_inputs("axis_group", "axis_name", axis_values_1, axes_utils.AxisUnits.NOT_SPECIFIED)
        axis_dict_2 = axes_utils.AxisObject.create_dict_inputs("axis_group", "axis_name", axis_values_2, axes_utils.AxisUnits.NOT_SPECIFIED)
        dict_dataset = datasets_utils.DatasetObject.create_dict_inputs("dataset_group", "dataset_name", dataset_values, datasets_utils.DatasetUnits.NOT_SPECIFIED)
    ...
    def test_adding_datasets_with_different_units(self):
        ...
        dataset_dict_1 = datasets_utils.DatasetObject.create_dict_inputs("dataset_group", "dataset_1", dataset_values_1, datasets_utils.DatasetUnits.DIMENSIONLESS)
    ...
    def test_global_axis_inheritance(self):
        self.dict_axis = axes_utils.AxisObject.create_dict_inputs("axis_group", "axis_name", [0, 1, 2], units=axes_utils.AxisUnits.DIMENSIONLESS, notes="original axis")
        self.dict_dataset = datasets_utils.DatasetObject.create_dict_inputs("dataset_group", "dataset_name", [10, 20, 30], units=datasets_utils.DatasetUnits.DIMENSIONLESS, notes="test dataset")
        self.obj_h5dm.add(self.dict_dataset, [self.dict_axis])
        new_units = axes_utils.AxisUnits.T_TURB
...
class TestHDF5DataManagerSaveLoad(unittest.TestCase):
    ...
    def test_valid_file_structure(self):
        ...
        try:
            dict_axis = axes_utils.AxisObject.create_dict_inputs("axis_group", "axis_name", [0, 1, 2], units=axes_utils.AxisUnits.T_TURB, notes="original axis")
            dict_dataset = datasets_utils.DatasetObject.create_dict_inputs("dataset_group", "dataset_name", [10, 20, 30], units=datasets_utils.DatasetUnits.DIMENSIONLESS, notes="test dataset")
    ...
    def test_save_load_multiple_datasets(self):
        axis1 = axes_utils.AxisObject.create_dict_inputs("group1", "axis1", [0, 1, 2])
        axis2 = axes_utils.AxisObject.create_dict_inputs("group2", "axis2", [3, 4, 5])
        dataset1 = datasets_utils.DatasetObject.create_dict_inputs("group1", "dataset1", [10, 20, 30])
        dataset2 = datasets_utils.DatasetObject.create_dict_inputs("group2", "dataset2", [40, 50, 60])
        obj_h5dm_save = h5dm.HDF5DataManager()
    ...
    def test_file_structure_validation(self):
        ...
        axis = axes_utils.AxisObject.create_dict_inputs("group", "axis", [0, 1, 2])
        dataset = datasets_utils.DatasetObject.create_dict_inputs("group", "dataset", [10, 20, 30])
